...
class SearchRestaurantByCuisineUseCaseTest {

    @Mock
    private SearchRestaurantAdapter searchRestaurantAdapter;

    @InjectMocks
    private SearchRestaurantByCuisineUseCase searchRestaurantByCuisineUseCase;
    ...
    @Test
    void sucessSearchCuisine() {

        List<Restaurant> mockRestaurants = new ArrayList<>();
        mockRestaurants.add(createRestaurant());
        when(searchRestaurantAdapter.findAllByCuisine(Cuisine.ITALIAN)).thenReturn(mockRestaurants);

        RestaurantListOutput result = searchRestaurantByCuisineUseCase.execute(Cuisine.ITALIAN);
        assertNotNull(result);
        assertThat(result.getBody().get(0).getCuisine()).isEqualTo(Cuisine.ITALIAN);
        assertThat(result.getOutputStatus().getCode()).isEqualTo(200);
        assertThat(result.getOutputStatus().getCodeName()).isEqualTo("OK");
        assertThat(result.getOutputStatus().getMessage()).isEqualTo("Restaurants found successfully");


    }

    @Test
    void SearchCuisineIsNotExist() {

        List<Restaurant> mockRestaurants = new ArrayList<>();
        mockRestaurants.add(createRestaurant());
        when(searchRestaurantAdapter.findAllByCuisine(Cuisine.ITALIAN)).thenReturn(mockRestaurants);

        RestaurantListOutput result = searchRestaurantByCuisineUseCase.execute(Cuisine.CHINESE);
        assertNotNull(result);
