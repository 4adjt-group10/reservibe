...
class SearchRestaurantByCityUseCaseTest {
    @Mock
    private SearchRestaurantAdapter searchRestaurantAdapter;

    @InjectMocks
    private SearchRestaurantByCityUseCase searchRestaurantByCityUseCase;
    ...
    @Test
    void sucessSearchCity() {
        List<Restaurant> mockRestaurants = new ArrayList<>();
        mockRestaurants.add(createRestaurant());
        when(searchRestaurantAdapter.findAllByCity("city")).thenReturn(mockRestaurants);

        RestaurantListOutput result = searchRestaurantByCityUseCase.execute("city");
        assertNotNull(result);
        assertThat(result.getBody().get(0).getAddress().getCity()).isEqualTo("city");
    }

    @Test
    void searchCityNotContent() {
        List<Restaurant> mockRestaurants = new ArrayList<>();
        when(searchRestaurantAdapter.findAllByCity("city")).thenReturn(mockRestaurants);
